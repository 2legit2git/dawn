...
static bool sketch_is_empty(const TexSketch* s)
{
    if (!s || s->height == 0)
        return true;
    if (s->height == 1 && s->rows[0].count == 0)
        return true;
    return false;
}
...
static TexTokenType get_char_type(char c)
{
    ...
    if (ISDIGIT_(c))
        return TEX_TOK_NUMB;
    if (strchr(SPECIAL_CHARS, c) || strchr(SYMBOL_CHARS, c))
        return TEX_TOK_SYMB;
    return TEX_TOK_SYMB;
}
...
TexToken* tex_lex(const char* input, size_t len, int32_t* out_count)
{
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (val_len > 1 && token_val[0] == '\\') {
            if (!is_final_char && char_type == get_char_type(tex[i + 1]) && char_type != TEX_TOK_SYMB) {
                continue;
            } else {
        ...
        if (token_type == TEX_TOK_SYMB && val_len == 1 && token_val[0] == ' ') {
            if (prev_token.type == TEX_TOK_NONE || (prev_token.type == TEX_TOK_SYMB && strcmp(prev_token.value, " ") == 0)) {
                val_len = 0;
    ...
    if (count > 0) {
        if (tokens[0].type == TEX_TOK_CMND && (strcmp(tokens[0].value, "[") == 0 || strcmp(tokens[0].value, "(") == 0)) {
            need_line_wrapper = false;
        }
        if (tokens[0].type == TEX_TOK_SYMB && (strcmp(tokens[0].value, "$") == 0 || strcmp(tokens[0].value, "$$") == 0)) {
            need_line_wrapper = false;
        }
...
static TexNodeType get_type_from_token(const TexToken* token)
{
    switch (token->type) {
    case TEX_TOK_META:
        if (strcmp(token->value, "start") == 0)
            return TEX_NT_OPN_ROOT;
        if (strcmp(token->value, "end") == 0)
            return TEX_NT_CLS_ROOT;
        if (strcmp(token->value, "startline") == 0)
            return TEX_NT_OPN_LINE;
        if (strcmp(token->value, "endline") == 0)
            return TEX_NT_CLS_LINE;
        break;
    case TEX_TOK_SYMB:
        switch (token->value[0]) {
        case '^':
            return TEX_NT_SUP_SCRPT;
        case '_':
            return TEX_NT_SUB_SCRPT;
        case '{':
            return TEX_NT_OPN_BRAC;
        case '}':
            return TEX_NT_CLS_BRAC;
        case ' ':
            return TEX_NT_TXT_INVS;
        case '$':
            return (token->value[1] == '$') ? TEX_NT_OPN_DDLR : TEX_NT_OPN_DLLR;
        }
        break;
    case TEX_TOK_CMND:
...
static TexNodeType get_node_type(const TexToken* token, TexNodeType parent_type)
{
    ...
    if (parent_type == TEX_NT_OPN_ENVN && (token->type == TEX_TOK_SYMB || token->type == TEX_TOK_ALPH || token->type == TEX_TOK_NUMB)) {
...
{
    return t == TEX_NT_SUP_SCRPT || t == TEX_NT_SUB_SCRPT || t == TEX_NT_TOP_SCRPT || t == TEX_NT_BTM_SCRPT;
}
...
{
    if (!is_script_type(node_type) || stack_count == 0)
        return -1;
...
{
    if (base_type != TEX_NT_CTR_BASE)
        return script_type;
    if (script_type == TEX_NT_SUP_SCRPT)
        return TEX_NT_TOP_SCRPT;
    if (script_type == TEX_NT_SUB_SCRPT)
        return TEX_NT_BTM_SCRPT;
    return script_type;
}
...
static TexSketch* util_vert_pile(TexSketch* top, TexSketch* ctr, int32_t ctr_horizon, TexSketch* btm, TexAlign align)
{
    ...
    int32_t max_len = 0;
    if (top && !sketch_is_empty(top) && top->rows[0].count > max_len)
        max_len = top->rows[0].count;
    if (ctr && !sketch_is_empty(ctr) && ctr->rows[0].count > max_len)
        max_len = ctr->rows[0].count;
    if (btm && !sketch_is_empty(btm) && btm->rows[0].count > max_len)
        max_len = btm->rows[0].count;
...
static TexSketch* util_shrink(TexSketch* sketch, int32_t script_type_id, bool smart, bool switch_script)
{
    ...
    for (int32_t i = 0; i < row->count; i++) {
        ...
        const char* target_script = (script_type_id == 0) ? tex_to_superscript(base_str) : tex_to_subscript(base_str);
        const char* other_script = (script_type_id == 0) ? tex_to_subscript(base_str) : tex_to_superscript(base_str);
...
static TexSketch* util_delimiter(const char* delim_type, int32_t height, int32_t horizon)
{
    ...
    for (int32_t i = 0; i < height; i++) {
        row_init(&rows[i]);
        TexDelimPos pos = (i == 0) ? TEX_DELIM_TOP : (i == height - 1) ? TEX_DELIM_BTM
            : (i == center)                                            ? TEX_DELIM_CTR
                                                                       : TEX_DELIM_FIL;
...
static TexSketch* render_apply_scripts(TexSketch* base, TexNodeArray* nodes, TexIdArray* script_ids, bool use_serif, bool inline_mode)
{
    ...
    if (top && top->height > 1) {
        ...
        if (top_shrunk && btm_shrunk) {
            ...
            if (top_shrunk)
                tex_sketch_free(top_shrunk);
            if (btm_shrunk)
                tex_sketch_free(btm_shrunk);
            ctr_new = sketch_empty();
            row_push(&ctr_new->rows[0], TEX_BG);
        }
    } else {
...
static TexSketch* render_fraction(TexNodeArray* nodes, int32_t node_id, bool use_serif, bool inline_mode)
{
    ...
    if (denom->rows[0].count > max_width)
        max_width = denom->rows[0].count;
...
static TexSketch* render_sqrt(TexNodeArray* nodes, int32_t node_id, bool use_serif, bool inline_mode)
{
    ...
    if (degree && !sketch_is_empty(degree) && degree->height == 1) {
        ...
        if (!shrunk_deg)
            shrunk_deg = sketch_clone(degree);
...
static TexSketch* render_binom(TexNodeArray* nodes, int32_t node_id, bool use_serif, bool inline_mode)
{
    ...
    if (inline_mode) {
        ...
        for (int32_t c = 0; c < n->rows[0].count; c++) {
            if (n->rows[0].cells[c].data)
                row_push(row, n->rows[0].cells[c].data);
        }
        row_push(row, ",");
        for (int32_t c = 0; c < r->rows[0].count; c++) {
            if (r->rows[0].cells[c].data)
                row_push(row, r->rows[0].cells[c].data);
        }
        row_push(row, ")");
        tex_sketch_free(n);
        tex_sketch_free(r);
    ...
    if (r->rows[0].count > max_width)
        max_width = r->rows[0].count;
...
static TexSketch* render_open_delim(TexNodeArray* nodes, int32_t node_id, bool use_serif, bool inline_mode)
{
    ...
    const char* left_char = (left_leaf && left_leaf->height > 0 && left_leaf->rows[0].count > 0) ? left_leaf->rows[0].cells[0].data : "(";
    const char* right_char = (right_leaf && right_leaf->height > 0 && right_leaf->rows[0].count > 0) ? right_leaf->rows[0].cells[0].data : ")";
    ...
    if (inline_mode) {
        ...
        if (inside && inside->height > 0) {
            for (int32_t c = 0; c < inside->rows[0].count; c++) {
                if (inside->rows[0].cells[c].data)
                    row_push(row, inside->rows[0].cells[c].data);
            }
        }
...
static TexSketch* render_big_delim(TexNodeArray* nodes, int32_t node_id, bool use_serif, bool inline_mode)
{
    ...
    const char* delim_type = (delim_leaf && delim_leaf->height > 0 && delim_leaf->rows[0].count > 0) ? delim_leaf->rows[0].cells[0].data : "(";
    ...
    if (strcmp(cmd, "Big") == 0 || strcmp(cmd, "Bigl") == 0 || strcmp(cmd, "Bigr") == 0)
        height = 3;
    else if (strcmp(cmd, "bigg") == 0 || strcmp(cmd, "biggl") == 0 || strcmp(cmd, "biggr") == 0)
        height = 5;
    else if (strcmp(cmd, "Bigg") == 0 || strcmp(cmd, "Biggl") == 0 || strcmp(cmd, "Biggr") == 0)
        height = 7;
        ...
        for (int32_t i = 0; i < node->children.count; i++) {
            ...
            if (child && child->height > 0) {
                for (int32_t c = 0; c < child->rows[0].count; c++) {
                    if (child->rows[0].cells[c].data)
                        row_push(row, child->rows[0].cells[c].data);
                }
            }
            tex_sketch_free(child);
        }
...
static TexSketch* render_node(TexNodeArray* nodes, int32_t node_id, bool use_serif, bool inline_mode)
{
    if (node_id < 0 || node_id >= nodes->count)
        return sketch_empty();
