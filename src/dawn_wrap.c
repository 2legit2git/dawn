...
{
    if (cp == NBSP)
        return false; // Non-breaking space is not breakable whitespace
    return cp == ' ' || cp == '\t' || cp == '\n' || cp == '\r' || cp == '\v' || cp == '\f' || cp == 0x0085 || // NEL
...
bool grapheme_is_wordy(const char* text, size_t len)
{
    ...
    // Numbers: Nd, Nl, No
    return (cat >= UTF8PROC_CATEGORY_LU && cat <= UTF8PROC_CATEGORY_LO) || (cat >= UTF8PROC_CATEGORY_ND && cat <= UTF8PROC_CATEGORY_NO);
}
...
static size_t next_grapheme_stateful(const char* text, size_t len, size_t pos, utf8proc_int32_t* state)
{
    ...
    if (bytes < 0)
        return pos + 1; // Invalid UTF-8, advance one byte
        ...
        if (is_newline(cp)) {
            ...
            if (config.trim_whitespace && line.width > 0) {
                ...
                while (scan > line.start) {
                    size_t prev = scan - 1;
                    while (prev > line.start && (text[prev] & 0xC0) == 0x80)
                        prev--;
                    utf8proc_int32_t prev_cp = codepoint_at(text, len, prev);
                    if (!is_whitespace(prev_cp))
                        break;
                    scan = prev;
        ...
        if (line.width + word.width > width) {
            ...
            if (word.width > width && config.split_words && !word.has_nbsp) {
                ...
                while (word_pos < word.end) {
                    ...
                    bool curr_wordy = (cat >= UTF8PROC_CATEGORY_LU && cat <= UTF8PROC_CATEGORY_LO) || (cat >= UTF8PROC_CATEGORY_ND && cat <= UTF8PROC_CATEGORY_NO);
